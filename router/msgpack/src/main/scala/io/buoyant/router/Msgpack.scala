...
import com.twitter.finagle.msgpack.{Request, Response}
...
object Msgpack extends Router[Request, Response] with Server[Request, Response] {
  ...
  object Router {
    ...
    val boundStack: Stack[ServiceFactory[Request, Response]] =
      StackRouter.newBoundStack[Request, Response]
        .replace(MsgpackEncodeResidual.role, MsgpackEncodeResidual)
  ...
  val server = Msgpack.Server(Server.stack, Server.defaultParams)
